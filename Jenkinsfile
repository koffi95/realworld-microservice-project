...
pipeline{
    ...
    stages {
        ...
        stage('Checkout from Git'){
            steps{
                git branch: 'dev-sec-ops-cicd-pipeline-project-one', url: 'https://github.com/koffi95/realworld-microservice-project.git'
            }
        }
        ...
        stage("Trivy App Image Scan"){
            steps{
                sh "trivy image koffi95/reddit:latest > trivy_image_analysis_report.txt"
            }
        }
        ...
        stage('ZAP Dynamic Testing | DAST') {
            steps {
                sshagent(['OWASP-Zap-Credential']) {
                    sh 'ssh -o StrictHostKeyChecking=no ubuntu@18.220.105.117 "docker run -t owasp/zap2docker-stable zap-baseline.py -t http://13.59.197.247:30000/" || true'
